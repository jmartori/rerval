...
#  }
  ...
  for (i in 0:(num_msg-1)){
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),1] <- from_line_to_time(res_snd[policy(l, na.rm = T)])
    }
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),2] <- from_line_to_time(res_snd[policy(l, na.rm = T)])
    }
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),3] <- from_line_to_time(res_rcv[policy(l, na.rm = T)])
    }
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),4] <- from_line_to_time(res_rcv[policy(l, na.rm = T)])
    }
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),5] <- from_line_to_time(res_rcv[policy(l, na.rm = T)])
    }
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),6] <- from_line_to_time(res_rcv[policy(l, na.rm = T)])
    ...
    if (len == 0) {
      ...
      m.ts[(i+1),7] <- from_line_to_time(res_snd[policy(l, na.rm = T)])
    }
  }
...
get_vfitting <- function(v_files, mc.enabled = FALSE, mc.cores = ifelse(mc.enabled, detectCores(), NA), res.sapplied = TRUE, ...){
  if (mc.enabled){
    mclapply(1:mc.cores, mc.cores = mc.cores, function(threadId){
      vv_files <- v_files[(1:length(v_files) %% mc.cores) == (threadId-1)]
      v_dir <- temp.dir(path = tempdir(), n = 1:mc.cores)
      
      sapply(vv_files, function(tfn){
        ret <- get_fitting(tfn = tfn, dir = v_dir[threadId], ...)
        cat(sprintf("%d %s\n", threadId, tfn))
    ...
    if (res.sapplied) {
      res <- sapply(1:length(v_files), function(i) {
        x <- (i %% mc.cores)+1
        y <- ceiling(i/mc.cores)
        res[[x]][,y]
      })
    }
    return(res)
