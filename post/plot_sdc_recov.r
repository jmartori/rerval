...
v.wt0.01 <- get_summary.mean(get_summary(v_fnn))[2:11,4]

v_fnn <- get_filenames(dir=path, pattern="sdc-loss_0.01-wt_0.1-ro_10-*")
v.wt0.1 <- get_summary.mean(get_summary(v_fnn))[2:11,4]

v_fnn <- get_filenames(dir=path, pattern="sdc-loss_0.01-wt_1-ro_10-*")
v.wt1 <- get_summary.mean(get_summary(v_fnn))[2:11,4]
...
matplot(m.mean, type='b', lty=1, ylab="#Recovered Operations", xaxt="n", xlab="Different Waiting Times (wt) ")
